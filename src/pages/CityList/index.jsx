...
class CityList extends Component {
    ...
    rowRenderer = ({
        ...
        return (
            <div key={key} style={style}>
                ...
                {citys.map(item => <div onClick={() => { this.handleRowClick(item) }} key={item.value} className='city'>{item.label}</div>)}
            </div>
        );
    }
    async initCitys() {
    ...
    renderRightList() {
        ...
        return cityIndex.map((item, index) => {
            ...
            }} className={activeIndex === index ? 'active' : ''} key={item}>{item === 'hot' ? '热' : item.toUpperCase()}</li>
        })
    }
